...
class TestPasswordConfigured:
    ...
    def test_wrong_credentials_are_rejected(self, client, monkeypatch) -> None:
        monkeypatch.setenv("AUTH_PASSWORD", "secret")
        response = client.get("/api/board", headers=_basic_header("play", "not-it"))
        assert response.status_code == 401

    def test_correct_credentials_pass_through(self, client, monkeypatch) -> None:
        monkeypatch.setenv("AUTH_PASSWORD", "secret")
        response = client.get("/api/board", headers=_basic_header("play", "secret"))
        assert response.status_code == 200

    def test_custom_username_is_honored(self, client, monkeypatch) -> None:
        monkeypatch.setenv("AUTH_PASSWORD", "secret")
        monkeypatch.setenv("AUTH_USERNAME", "admin")
        assert client.get("/api/board", headers=_basic_header("play", "secret")).status_code == 401
        assert client.get("/api/board", headers=_basic_header("admin", "secret")).status_code == 200
